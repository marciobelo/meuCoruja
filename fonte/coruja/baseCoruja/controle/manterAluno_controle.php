<?php
    ...
    if($acao=="consultar") {
        ...
        if(!$usuario->temPermissao($MANTER_ALUNO_CONSULTAR)) {
        ...
        // Verifica antes se usuário tem permissão
        $usuario = $_SESSION["usuario"];
        if(!$usuario->temPermissao($MANTER_ALUNO_INSERIR)) {
            require_once("$BASE_DIR/baseCoruja/formularios/sem_permissao.php");
        }
        ...
        try {
            ...
            $usuario->incluirLog($MANTER_ALUNO_INSERIR,$strLog,$con);
        ...
        // Verifica antes se usuário tem permissão
        $usuario = $_SESSION["usuario"];
        if(!$usuario->temPermissao($MANTER_ALUNO_EDITAR)) {
            require_once("$BASE_DIR/baseCoruja/formularios/sem_permissao.php");
        }
        ...
        // TODO MB ver problema do campo Aluno.estadoCivil
        // após alteração do mysql mode, sendo mais restritivo,
        // passou a dar erro em dados carregados incorretamente da massa
        // de teste
        ...
        try {
            ...
            $usuario->incluirLog($MANTER_ALUNO_EDITAR,$strLog,$con);
        ...
        // Verifica antes se usuário tem permissão
        $usuario = $_SESSION["usuario"];
        if(!$usuario->temPermissao($MANTER_ALUNO_EDITAR_MATRICULA)) {
            require_once("$BASE_DIR/baseCoruja/formularios/sem_permissao.php");
        }
        ...
        // Verifica antes se usuário tem permissão
        if(!$usuario->temPermissao($MANTER_ALUNO_INCLUIR_MATRICULA)) {
            require_once("$BASE_DIR/baseCoruja/formularios/sem_permissao.php");
        }
        ...
        // Verifica antes se usuário tem permissão
        if(!$usuario->temPermissao($MANTER_ALUNO_MUDAR_SITUACAO_DOC_ENTREGUE)) {
            require_once("$BASE_DIR/baseCoruja/formularios/sem_permissao.php");
        }
        ...
        try {
            ...
            $usuario->incluirLog($MANTER_ALUNO_MUDAR_SITUACAO_DOC_ENTREGUE,  $strLog, $con);
